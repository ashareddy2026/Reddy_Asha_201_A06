...
```{r}
rounded_points_per_game = c(lapply(points_per_game, round, digits = 2))
...
rounded_mat <- apply(mat_rand, MARGIN = 1:2, custom_round)
rounded_mat
```
...
```{r}
max_no_state = max_temps[, c(-1,-2)]
month_mat = apply(max_no_state, 2, mean, na.rm = TRUE)
month_mat
max_temps
```
...
```{r}
mat_co = max_temps[max_temps$state == "CO",]
mat_oh = max_temps[max_temps$state == "OH",]
mat_va = max_temps[max_temps$state == "VA",]
```
...
```{r}
mat_co2 = mat_co[, c(-1,-2)]
medians = apply(mat_co2, 1, median)
paste0("The maximum medium annual temperature of ", medians[which.max(medians)], "occured in ", mat_co$year[which.max(medians)], " and the minimum median annual temperature of ", medians[which.min(medians)], "occured in ", mat_co$year[which.min(medians)], ".")
```
...
```{r}
add_dig = function(x, count = 0){
  if (x == 0){
    return(count)
  }
  count = count + (x %% 10)
  return(add_dig(x %/% 10, count))
}
